...
C     --- Gauss point field values ---
    ...
    lines.append('      END IF')
    lines.append('')
    lines.append('C     Procedure dispatch (Abaqus UEL contract): supported are the')
    lines.append('C     static/quasi-static and coupled thermal-stress procedures')
    lines.append('C     (LFLAGS(1)=1,2,71,72,73), general non-perturbation steps')
    lines.append('C     (LFLAGS(4)=0), NRHS=1 (modified Riks is not implemented),')
    lines.append('C     and request types LFLAGS(3)=1,2,5. Types 2 and 5 receive')
    lines.append('C     the full residual+stiffness evaluation, a superset of the')
    lines.append('C     requested output; the unused array is ignored by Abaqus.')
    lines.append('C     Every other request returns the zeroed arrays.')
    lines.append('      IF (LFLAGS(1) .NE. 1 .AND. LFLAGS(1) .NE. 2 .AND.')
    lines.append('     &    LFLAGS(1) .NE. 71 .AND. LFLAGS(1) .NE. 72 .AND.')
    lines.append('     &    LFLAGS(1) .NE. 73) THEN')
    lines.append('        RETURN')
    lines.append('      END IF')
    lines.append('      IF (LFLAGS(4) .NE. 0) THEN')
    lines.append('        RETURN')
    lines.append('      END IF')
    lines.append('      IF (NRHS .GE. 2) THEN')
    lines.append('        RETURN')
    lines.append('      END IF')
    lines.append('      IF (LFLAGS(3) .NE. 1 .AND. LFLAGS(3) .NE. 2 .AND.')
    lines.append('     &    LFLAGS(3) .NE. 5) THEN')
    lines.append('        RETURN')
    lines.append('      END IF')
    lines.append('')
    ...
    for fname in field_names:
        ...
        if isinstance(f, VectorField):
            ...
            lines.append(f'        END DO')
            lines.append('')
            lines.append('C       Reject invalid deformation states: det F must stay')
            lines.append('C       positive, else request a cutback (guards complex-step')
            lines.append('C       evaluation of log/inverse at meaningless states).')
            lines.append('        IF (det33d(F) .LE. 0.0d0) THEN')
            lines.append('          PNEWDT = 0.25d0')
            lines.append('          RETURN')
            lines.append('        END IF')
            lines.append('')
