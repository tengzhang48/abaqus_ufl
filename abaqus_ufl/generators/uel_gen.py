...
C     --- Gauss point field values ---
    ...
    lines.append('      END IF')
    lines.append('')
    lines.append('C     Procedure dispatch (Abaqus UEL contract): only the static,')
    lines.append('C     quasi-static, and coupled thermal-stress procedures')
    lines.append('C     (LFLAGS(1)=1,2,71,72,73) with residual/stiffness request')
    lines.append('C     types LFLAGS(3)=1,2,5 are supported. Any other request')
    lines.append('C     returns the zeroed arrays (no dynamics, perturbation, or')
    lines.append('C     mass output is claimed).')
