...
def _generate_uel_local_pressure(weakform, mat_prefix, cfg):
    ...
    lines.append('          RETURN')
    lines.append('        END IF')
    lines.append('        IF (R_p .NE. R_p .OR. K_pp .NE. K_pp .OR.')
    lines.append('     &      DABS(R_p) .GT. 1.0d30 .OR.')
    lines.append('     &      DABS(K_pp) .GT. 1.0d30) THEN')
    lines.append('          PNEWDT = 0.25d0')
    lines.append('          RETURN')
    lines.append('        END IF')
    lines.append('        dp = -R_p / K_pp')
    lines.append('        p_elem = p_elem + dp')
    lines.append('        IF (DABS(R_p) .LT. 1.0d-10) THEN')
    lines.append('          conv_p = 1')
    lines.append('          EXIT')
    lines.append('        END IF')
    lines.append('        IF (DABS(dp) .LT. 1.0d-10*(1.0d0+DABS(p_elem))) THEN')
    lines.append('          conv_p = 1')
    lines.append('          EXIT')
    lines.append('        END IF')
    lines.append('      END DO')
    lines.append('C     Unconverged local solve: request a smaller increment,')
    lines.append('C     matching the global solver response to non-convergence.')
    lines.append('      IF (conv_p .EQ. 0) THEN')
    lines.append('        PNEWDT = 0.25d0')
    lines.append('        RETURN')
    lines.append('      END IF')
    lines.append('')
    ...
    lines.append('        IF (LFLAGS(3) .EQ. 1 .AND. NSVARS .GE. 1 + NGP)')
    lines.append('     &    SVARS(1+kk) = phi_diag')
    lines.append('        IF (LFLAGS(3) .EQ. 1 .AND. NSVARS .GE. 1 + 2*NGP)')
    lines.append('     &    SVARS(1+NGP+kk) = p_elem')
    lines.append('        R_p = R_p + rp_real*wdetJ')
    lines.append('        K_pp = K_pp + drp_dp*wdetJ')
    lines.append('')
    # RHS u
    ...
    lines.append('        END DO')
    lines.append('      END DO')
    lines.append('      IF (LFLAGS(3) .EQ. 1 .AND. NSVARS .GE. 1)')
    ...
    lines.append('      IF (LFLAGS(3) .EQ. 1 .AND. ndim .EQ. 2')
    lines.append('     &    .AND. NSVARS .GE. 32) THEN')
    lines.append('        SVARS(1) = p_elem')
