...
C     Generated by abaqus_ufl for material: smallstrainj2
C     NPROPS = 4
C     NSTATV = 1
C======================================================================
...
C     --- Real stress update ---
      ...
      CALL smallstrainj2_stress_update(sigma_old_z,
     &  strain_old_z,
     &  dstrain_z,
     &  ep_old_z,
     &  dt_safe,
...
C======================================================================
C     Section 2: Material stress update: smallstrainj2_stress_update
C======================================================================
      SUBROUTINE smallstrainj2_stress_update(sigma_old, strain_old,
     &dstrain, ep_old, dt, cutback_flag, props, sigma_new, ep_new)
      IMPLICIT NONE
      ...
      DOUBLE COMPLEX :: DEV3(3,3)
      DOUBLE COMPLEX :: AT4(3,3)
      DOUBLE COMPLEX :: MM5(3,3)
      DOUBLE COMPLEX :: EYE6(3,3)
      DOUBLE COMPLEX :: DEV7(3,3)
      DOUBLE COMPLEX :: n(3,3)
      DOUBLE COMPLEX :: seq
      DOUBLE COMPLEX :: f
      DOUBLE COMPLEX :: dgamma
      INTEGER :: ii, jj

      cutback_flag = DCMPLX(0.0d0, 0.0d0)

      CALL eye33z(EYE1)
      sigma_trial = ((sigma_old + ((DCMPLX(2.000000000000000d+00,
     &0.0d0) * DCMPLX(props(1), 0.0d0)) * dstrain)) + ((DCMPLX(props(2),
     & 0.0d0) * (dstrain(1,1)+dstrain(2,2)+dstrain(3,3))) * EYE1))
      CALL eye33z(EYE2)
      DEV3 = sigma_trial - ((sigma_trial(1,1)+sigma_trial(2,2)+
     &sigma_trial(3,3)) / DCMPLX(3.000000000000000d+00, 0.0d0)) * EYE2
      CALL transpose33z(DEV3, AT4)
      CALL matmul33z(AT4, DEV3, MM5)
      seq = SQRT(DCMPLX(1.500000000000000d+00, 0.0d0) * (MM5(1,1)+MM5(2,
     &2)+MM5(3,3)))
      f = (seq - (DCMPLX(props(3), 0.0d0) + (DCMPLX(props(4), 0.0d0) *
     &ep_old)))
      IF (DBLE(f) .GT. 0.000000000000000d+00) THEN
      dgamma = (f / ((DCMPLX(3.000000000000000d+00, 0.0d0) *
     &DCMPLX(props(1), 0.0d0)) + DCMPLX(props(4), 0.0d0)))
      CALL eye33z(EYE6)
      DEV7 = sigma_trial - ((sigma_trial(1,1)+sigma_trial(2,2)+
     &sigma_trial(3,3)) / DCMPLX(3.000000000000000d+00, 0.0d0)) * EYE6
      n = (DCMPLX(1.500000000000000d+00, 0.0d0) / (seq +
     &DCMPLX(1.000000000000000d-30, 0.0d0))) * DEV7
      sigma_new = (sigma_trial - (((DCMPLX(2.000000000000000d+00,
     &0.0d0) * DCMPLX(props(1), 0.0d0)) * dgamma) * n))
      ...
      RETURN
      END SUBROUTINE smallstrainj2_stress_update
