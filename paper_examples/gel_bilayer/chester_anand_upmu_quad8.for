...
C     Material: ChesterAnandGelMaterial
C     Fields: u, p, mu
C     Element: Quad8, NDOFEL = 28
C======================================================================
...
C     --- Local parameters ---
      ...
      INTEGER :: edof_p(4)
      DOUBLE PRECISION :: mu_node(8)
      DOUBLE PRECISION :: mu_old(8)
      INTEGER :: edof_mu(8)
...
C     --- Gauss point field values ---
      ...
      DOUBLE PRECISION :: mu_gp, mu_old_gp
      DOUBLE PRECISION :: grad_mu(3)
...
C     Complex temps for real material evaluation
      ...
      DOUBLE COMPLEX :: pz_r
      DOUBLE COMPLEX :: muz_r
      DOUBLE COMPLEX :: grad_mu_r(3)

C     --- Tangent arrays ---
      DOUBLE PRECISION :: dPK1_dF(3,3,3,3)
      DOUBLE PRECISION :: dPK1_dmu(3,3)
      DOUBLE PRECISION :: dPK1_dp(3,3)
      DOUBLE PRECISION :: drp_resid_dF(3,3)
      DOUBLE PRECISION :: drp_resid_dmu
      DOUBLE PRECISION :: drp_resid_dp
      DOUBLE PRECISION :: dflux_dF(3,3,3)
      DOUBLE PRECISION :: dflux_dgrad_mu(3,3)
      DOUBLE PRECISION :: dflux_dmu(3)
      DOUBLE PRECISION :: dflux_dp(3)
      DOUBLE PRECISION :: dstorage_dF(3,3)
      DOUBLE PRECISION :: dstorage_dp
...
C       evaluation of log/inverse at meaningless states).
     ...
     &      + p_old(ii_v)*sh4(ii_v)
        ...
        DO ii_v = 1, 8
          mu_gp = mu_gp
     &      + mu_node(ii_v)*sh8(ii_v)
          mu_old_gp = mu_old_gp
     &      + mu_old(ii_v)*sh8(ii_v)
        END DO
        grad_mu(1) = 0.0d0
        grad_mu(2) = 0.0d0
        grad_mu(3) = 0.0d0
...
C       Real material evaluation (for RHS)
        ...
        muz_r = DCMPLX(mu_gp, 0.0d0)
        CALL real2complex3(grad_mu, grad_mu_r)
        CALL chesteranandupmu_stress_PK1(Fz_r, pz_r, muz_r, PROPS,
     &Pz_eval)
        DO i = 1, 3
          DO j = 1, 3
          ...
          END DO
        END DO
        CALL chesteranandupmu_pressure_resid(Fz_r, pz_r, muz_r, PROPS,
     &rpz_eval)
        rp_real = DBLE(rpz_eval)
        CALL chesteranandupmu_solvent_flux(Fz_r, pz_r, muz_r, grad_mu_r,
     & PROPS, jRz_eval)
        DO i = 1, 3
          jR_real(i) = DBLE(jRz_eval(i))
        END DO
        CALL chesteranandupmu_solvent_storage(Fz_r, Fold_r, pz_r,
     &DCMPLX(p_old_gp, 0.0d0), dt_safe, PROPS, cdotz_eval)
        cdot_real = DBLE(cdotz_eval)

C       CS tangent engine
        CALL chesteranandupmu_cs_tangents(F, mu_gp, p_gp, grad_mu,
     &     F_old, p_old_gp, PROPS, dt_safe, dPK1_dF, dPK1_dmu,
     &     dPK1_dp, drp_resid_dF, drp_resid_dmu, drp_resid_dp,
     &     dflux_dF, dflux_dgrad_mu, dflux_dmu, dflux_dp,
...
C       RHS assembly
     ...
     &      - rp_real*sh4(ii_v)*wdetJ
        ...
        DO ii_v = 1, 8
          row = edof_mu(ii_v)
          RHS(row,1) = RHS(row,1)
     &      - cdot_real*sh8(ii_v)*wdetJ
          DO j = 1, ndim
            RHS(row,1) = RHS(row,1)
     &        + jR_real(j)*dsh8(ii_v,j)*wdetJ
          END DO
        END DO
...
C       drp_resid_dmu (sign=+1, value-value)
        DO ii_v = 1, 4
          DO jj_v = 1, 8
            row = edof_p(ii_v)
            col = edof_mu(jj_v)
...
C       dflux_dgrad_mu (sign=-1, grad-grad)
        DO ii_v = 1, 8
          DO jj_v = 1, 8
            row = edof_mu(ii_v)
            col = edof_mu(jj_v)
...
C       dstorage_dp (sign=+1, value-value)
        DO ii_v = 1, 8
          DO jj_v = 1, 4
            row = edof_mu(ii_v)
            col = edof_p(jj_v)
...
C======================================================================
      SUBROUTINE chesteranandupmu_stress_PK1(F, p, mu, props, P_out)
      IMPLICIT NONE
     ...
     &jj))) + (p * AT4(ii,jj)))
      ...
      RETURN
      END SUBROUTINE chesteranandupmu_stress_PK1

      SUBROUTINE chesteranandupmu_pressure_resid(F, p, mu, props,
     &rp_out)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN) :: F(3,3)
      DOUBLE COMPLEX, INTENT(IN) :: p
      ...
      RT = (DCMPLX(props(7), 0.0d0) * DCMPLX(props(8), 0.0d0))
      rp_out = (((mu - DCMPLX(props(5), 0.0d0)) - (RT *
     ...
     &(((phi / DCMPLX(props(9), 0.0d0)) * DCMPLX(props(6), 0.0d0)) * p))
      ...
      RETURN
      END SUBROUTINE chesteranandupmu_pressure_resid

      SUBROUTINE chesteranandupmu_solvent_flux(F, p, mu, grad_mu, props,
     & jR_out)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN) :: F(3,3)
      DOUBLE COMPLEX, INTENT(IN) :: p
     ...
     &DCMPLX(props(8), 0.0d0)))
      ...
      RETURN
      END SUBROUTINE chesteranandupmu_solvent_flux

      SUBROUTINE chesteranandupmu_solvent_storage(F, F_old, p, p_old,
     &dt, props, cdot_out)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN) :: F(3,3)
     ...
     &0.0d0) * dt))
      ...
      RETURN
      END SUBROUTINE chesteranandupmu_solvent_storage


C======================================================================
C     Section 3: Complex-step tangent engine
C======================================================================
      SUBROUTINE chesteranandupmu_cs_tangents(
     &   F, mu, p, grad_mu, F_old, p_old, props, dt, dPK1_dF,
     &   dPK1_dmu, dPK1_dp, drp_resid_dF, drp_resid_dmu,
     &   drp_resid_dp, dflux_dF, dflux_dgrad_mu, dflux_dmu,
     &   dflux_dp, dstorage_dF, dstorage_dp)
...
C     === dPK1_dF: d(stress_PK1)/d(F) ===
      DO k = 1, 3
        DO l = 1, 3
          ...
          CALL chesteranandupmu_stress_PK1(Fz, pz, muz, props, PKz)
          DO i = 1, 3
            DO j = 1, 3
...
C     === dPK1_dmu: d(stress_PK1)/d(mu) ===
      ...
      DO i = 1, 3
        DO j = 1, 3
          dPK1_dmu(i,j) = AIMAG(PKz(i,j)) / CS_H
        END DO
      END DO
...
C     === drp_resid_dF: d(pressure_resid)/d(F) ===
      DO k = 1, 3
        DO l = 1, 3
          ...
          CALL real2complex3(grad_mu, grad_mu_z)
          Fz(k,l) = Fz(k,l) + DCMPLX(0.0d0, CS_H)
          CALL chesteranandupmu_pressure_resid(Fz, pz, muz, props, rpz)
          drp_resid_dF(k,l) = AIMAG(rpz) / CS_H
        END DO
      END DO

C     === drp_resid_dmu: d(pressure_resid)/d(mu) ===
      CALL real2complex33(F, Fz)
      muz = DCMPLX(mu, CS_H)
      pz = DCMPLX(p, 0.0d0)
      CALL real2complex3(grad_mu, grad_mu_z)
      CALL chesteranandupmu_pressure_resid(Fz, pz, muz, props, rpz)
      drp_resid_dmu = AIMAG(rpz) / CS_H
...
C     === dflux_dF: d(solvent_flux)/d(F) ===
      DO k = 1, 3
        DO l = 1, 3
          ...
          CALL real2complex3(grad_mu, grad_mu_z)
          Fz(k,l) = Fz(k,l) + DCMPLX(0.0d0, CS_H)
          CALL chesteranandupmu_solvent_flux(Fz, pz, muz, grad_mu_z,
     &props, jRz)
          DO i = 1, 3
...
C     === dflux_dgrad_mu: d(solvent_flux)/d(grad_mu) ===
      DO k = 1, 3
        CALL real2complex33(F, Fz)
        muz = DCMPLX(mu, 0.0d0)
        pz = DCMPLX(p, 0.0d0)
     ...
     &props, jRz)
        DO i = 1, 3
          dflux_dgrad_mu(i,k) = AIMAG(jRz(i)) / CS_H
        END DO
      END DO

C     === dflux_dmu: d(solvent_flux)/d(mu) ===
      CALL real2complex33(F, Fz)
      muz = DCMPLX(mu, CS_H)
      pz = DCMPLX(p, 0.0d0)
      CALL real2complex3(grad_mu, grad_mu_z)
      CALL chesteranandupmu_solvent_flux(Fz, pz, muz, grad_mu_z, props,
     &jRz)
      DO i = 1, 3
        dflux_dmu(i) = AIMAG(jRz(i)) / CS_H
      END DO

C     === dflux_dp: d(solvent_flux)/d(p) ===
      CALL real2complex33(F, Fz)
      muz = DCMPLX(mu, 0.0d0)
      pz = DCMPLX(p, CS_H)
      CALL real2complex3(grad_mu, grad_mu_z)
...
C     === dstorage_dF: d(solvent_storage)/d(F) ===
      DO k = 1, 3
        DO l = 1, 3
          ...
          CALL real2complex3(grad_mu, grad_mu_z)
          Fz(k,l) = Fz(k,l) + DCMPLX(0.0d0, CS_H)
          CALL chesteranandupmu_solvent_storage(Fz, F_old_z, pz,
     &p_old_z, dt, props, cdotz)
          dstorage_dF(k,l) = AIMAG(cdotz) / CS_H
        END DO
      END DO

C     === dstorage_dp: d(solvent_storage)/d(p) ===
      CALL real2complex33(F, Fz)
      muz = DCMPLX(mu, 0.0d0)
      pz = DCMPLX(p, CS_H)
      CALL real2complex3(grad_mu, grad_mu_z)
      CALL chesteranandupmu_solvent_storage(Fz, F_old_z, pz, p_old_z,
     &dt, props, cdotz)
      dstorage_dp = AIMAG(cdotz) / CS_H

      RETURN
      END SUBROUTINE chesteranandupmu_cs_tangents
