...
C     Material: ScovazziBlockMaterial
C     Fields: u, thetat
C     Element: Tet4, NDOFEL = 16
C======================================================================
...
C     --- Local parameters ---
      ...
      DOUBLE PRECISION :: thetat_node(4)
      DOUBLE PRECISION :: thetat_old(4)
      INTEGER :: edof_thetat(4)

C     --- Shape functions and mapping ---
      DOUBLE PRECISION :: sh4(4), dshxi4(4,3), dsh4(4,3)
      DOUBLE PRECISION :: xi_gp(4,3), w_gp(4)
      DOUBLE PRECISION :: coords_3d(3, NNODE_E)
      DOUBLE PRECISION :: detJ, Jinv(3,3), wdetJ
...
C     --- Gauss point field values ---
      ...
      DOUBLE PRECISION :: thetat_gp, thetat_old_gp
      DOUBLE PRECISION :: grad_thetat(3)
...
C     Complex temps for real material evaluation
      ...
      DOUBLE COMPLEX :: thetatz_r
      DOUBLE COMPLEX :: grad_thetat_r(3)

C     --- Tangent arrays ---
      DOUBLE PRECISION :: dPK1_dF(3,3,3,3)
      DOUBLE PRECISION :: dPK1_dthetat(3,3)
      DOUBLE PRECISION :: dphase_flux_dF(3,3,3)
      DOUBLE PRECISION :: dphase_flux_dgrad_thetat(3,3)
      DOUBLE PRECISION :: dphase_flux_dthetat(3)
      DOUBLE PRECISION :: dphase_storage_dF(3,3)
      DOUBLE PRECISION :: dphase_storage_dthetat

      INTEGER :: idx, ii_v, jj_v, i, j, k, l, kk, row, col

C     Extract 3D coordinates from COORDS(MCRD, NNODE)
      DO ii_v = 1, NNODE_E
...
C     Parse DOFs from U and DU, build edof maps
      ...
      DO ii_v = 1, NNODE_E
        ...
        IF (ii_v .LE. NCORNER) THEN
          idx = idx + 1
          thetat_node(ii_v) = U(idx)
          thetat_old(ii_v) = U(idx) - DU(idx, 1)
          edof_thetat(ii_v) = idx
        END IF
      END DO

C     Gauss quadrature
      CALL gauss_tet4(xi_gp, w_gp, ngp_out)

      DO kk = 1, ngp_out

C       Shape functions + geometry mapping
        CALL shape_tet4(xi_gp(kk,1), xi_gp(kk,2), xi_gp(kk,3),
     &                   sh4, dshxi4)
        CALL map_grad_3d(dshxi4, coords_3d, 4,
     &                   dsh4, detJ, Jinv, stat)
...
C       Trap degenerate Jacobian
        ...
        CALL shape_tet4(xi_gp(kk,1), xi_gp(kk,2), xi_gp(kk,3),
     &                   sh4, dshxi4)
        CALL apply_jinv_3d(dshxi4, Jinv, 4, dsh4)
...
C       evaluation of log/inverse at meaningless states).
        ...
        DO ii_v = 1, 4
          thetat_gp = thetat_gp
     &      + thetat_node(ii_v)*sh4(ii_v)
          thetat_old_gp = thetat_old_gp
     &      + thetat_old(ii_v)*sh4(ii_v)
        END DO
        grad_thetat(1) = 0.0d0
        grad_thetat(2) = 0.0d0
        grad_thetat(3) = 0.0d0
        DO ii_v = 1, 4
          DO j = 1, ndim
            grad_thetat(j) = grad_thetat(j)
     &        + thetat_node(ii_v)*dsh4(ii_v,j)
          END DO
        END DO
...
C       Real material evaluation (for RHS)
        ...
        thetatz_r = DCMPLX(thetat_gp, 0.0d0)
        CALL real2complex3(grad_thetat, grad_thetat_r)
        CALL scovazziblockmaterial_stress_PK1(Fz_r, thetatz_r, PROPS,
     &Pz_eval)
        DO i = 1, 3
          DO j = 1, 3
          ...
          END DO
        END DO
        CALL scovazziblockmaterial_phase_flux(Fz_r, thetatz_r,
     &grad_thetat_r, PROPS, phase_fluxz_eval)
        DO i = 1, 3
          phase_flux_real(i) = DBLE(phase_fluxz_eval(i))
        END DO
        CALL scovazziblockmaterial_phase_storage(Fz_r, thetatz_r, PROPS,
     & phase_storagez_eval)
        phase_storage_real = DBLE(phase_storagez_eval)

C       CS tangent engine
        CALL scovazziblockmaterial_cs_tangents(F, thetat_gp,
     &     grad_thetat, F_old, PROPS, dPK1_dF, dPK1_dthetat,
     &     dphase_flux_dF, dphase_flux_dgrad_thetat,
     &     dphase_flux_dthetat, dphase_storage_dF,
     &     dphase_storage_dthetat)
...
C       RHS assembly
     ...
     &          - P_real(i,j)*dsh4(ii_v,j)*wdetJ
        ...
        DO ii_v = 1, 4
          row = edof_thetat(ii_v)
          RHS(row,1) = RHS(row,1)
     &      - phase_storage_real*sh4(ii_v)*wdetJ
          DO j = 1, ndim
            RHS(row,1) = RHS(row,1)
     &        + phase_flux_real(j)*dsh4(ii_v,j)*wdetJ
          END DO
        END DO
...
C       dPK1_dthetat (sign=+1, grad-value)
        DO ii_v = 1, 4
          DO jj_v = 1, 4
            col = edof_thetat(jj_v)
            DO i = 1, ndim
              row = edof_u(i, ii_v)
              DO j = 1, ndim
                AMATRX(row,col) =
...
C       dphase_flux_dgrad_thetat (sign=-1, grad-grad)
        DO ii_v = 1, 4
          DO jj_v = 1, 4
            row = edof_thetat(ii_v)
            col = edof_thetat(jj_v)
...
C       dphase_flux_dthetat (sign=-1, grad-value)
        DO ii_v = 1, 4
          DO jj_v = 1, 4
            row = edof_thetat(ii_v)
            col = edof_thetat(jj_v)
...
C       dphase_storage_dthetat (sign=+1, value-value)
        DO ii_v = 1, 4
          DO jj_v = 1, 4
            row = edof_thetat(ii_v)
            col = edof_thetat(jj_v)
...
C======================================================================
      SUBROUTINE scovazziblockmaterial_stress_PK1(F, thetat, props,
     &P_out)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN) :: F(3,3)
      DOUBLE COMPLEX, INTENT(IN) :: thetat
      DOUBLE PRECISION, INTENT(IN) :: props(5)
      DOUBLE COMPLEX, INTENT(OUT) :: P_out(3,3)

      DOUBLE COMPLEX :: det33z
      DOUBLE COMPLEX :: ARG1(3,3)
      DOUBLE COMPLEX :: ARG2
      DOUBLE COMPLEX :: S(3,3)
      DOUBLE COMPLEX :: Cbar_inv(3,3)
      DOUBLE COMPLEX :: theta
      DOUBLE COMPLEX :: J
      DOUBLE COMPLEX :: ARG3(3,3)
      DOUBLE COMPLEX :: ARG4
      DOUBLE COMPLEX :: H5(3,3)
      DOUBLE COMPLEX :: Dbar(3,3)
      DOUBLE COMPLEX :: dS_dtheta(3,3)
      DOUBLE COMPLEX :: MM6(3,3)
      DOUBLE COMPLEX :: bulk
      DOUBLE COMPLEX :: tau_theta
      INTEGER :: ii, jj

      ARG1 = F
      ARG2 = thetat
      CALL scovazziblockmaterial_S_enriched(ARG1, ARG2, props, S,
     &Cbar_inv, theta, J)
      ARG3 = Cbar_inv
      ARG4 = theta
      CALL scovazziblockmaterial_Dbar(ARG3, ARG4, props, H5)
      Dbar = H5
      dS_dtheta = (Dbar / (DCMPLX(3.000000000000000d+00, 0.0d0) *
     &theta))
      bulk = (DCMPLX(props(2), 0.0d0) + ((DCMPLX(2.000000000000000d+00,
     &0.0d0) * DCMPLX(props(1), 0.0d0)) / DCMPLX(3.000000000000000d+00,
     &0.0d0)))
      tau_theta = ((DCMPLX(props(4), 0.0d0) * DCMPLX(props(1), 0.0d0)) /
     & (DCMPLX(props(1), 0.0d0) + bulk))
      CALL matmul33z(F, (S - ((tau_theta * (theta - J)) * dS_dtheta)),
     &MM6)

      DO ii = 1, 3
        DO jj = 1, 3
          P_out(ii,jj) = MM6(ii,jj)
        END DO
      END DO

      RETURN
      END SUBROUTINE scovazziblockmaterial_stress_PK1

      SUBROUTINE scovazziblockmaterial_phase_flux(F, thetat,
     &grad_thetat, props, phase_flux_out)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN) :: F(3,3)
      DOUBLE COMPLEX, INTENT(IN) :: thetat
      DOUBLE COMPLEX, INTENT(IN) :: grad_thetat(3)
      DOUBLE PRECISION, INTENT(IN) :: props(5)
      ...
      DOUBLE COMPLEX :: H5(3,3)
      DOUBLE COMPLEX :: Dbar(3,3)
      DOUBLE COMPLEX :: MM6(3)
      DOUBLE COMPLEX :: gradient_term(3)
      DOUBLE COMPLEX :: tau_u
      INTEGER :: ii, jj

      ARG1 = F
      ARG2 = thetat
      CALL scovazziblockmaterial_S_enriched(ARG1, ARG2, props, S_unused,
     & Cbar_inv, theta, J)
      ARG3 = Cbar_inv
      ARG4 = theta
      CALL scovazziblockmaterial_Dbar(ARG3, ARG4, props, H5)
      Dbar = H5
      tau_u = ((DCMPLX(props(3), 0.0d0) * (DCMPLX(props(5), 0.0d0) **
     &DCMPLX(2.0d0, 0.0d0))) / (DCMPLX(2.000000000000000d+00, 0.0d0) *
     &DCMPLX(props(1), 0.0d0)))
      gradient_term = ((tau_u / DCMPLX(3.000000000000000d+00, 0.0d0)) *
     &(J / theta))
      CALL matvec33z(Dbar, grad_thetat, MM6)
      gradient_term = (gradient_term * MM6)

      DO ii = 1, 3
        phase_flux_out(ii) = ((-DCMPLX(1.000000000000000d+00, 0.0d0)) *
     &gradient_term(ii))
      END DO

      RETURN
      END SUBROUTINE scovazziblockmaterial_phase_flux

      SUBROUTINE scovazziblockmaterial_phase_storage(F, thetat, props,
     &phase_storage_out)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN) :: F(3,3)
      DOUBLE COMPLEX, INTENT(IN) :: thetat
      ...
      theta = (DCMPLX(1.000000000000000d+00, 0.0d0) + thetat)
      J = det33z(F)
      bulk = (DCMPLX(props(2), 0.0d0) + ((DCMPLX(2.000000000000000d+00,
     &0.0d0) * DCMPLX(props(1), 0.0d0)) / DCMPLX(3.000000000000000d+00,
     &0.0d0)))
      tau_theta = ((DCMPLX(props(4), 0.0d0) * DCMPLX(props(1), 0.0d0)) /
     & (DCMPLX(props(1), 0.0d0) + bulk))
      phase_storage_out = ((DCMPLX(1.000000000000000d+00, 0.0d0) -
     &tau_theta) * (theta - J))

      RETURN
      END SUBROUTINE scovazziblockmaterial_phase_storage

      SUBROUTINE scovazziblockmaterial_S_enriched(F, thetat, props,
     &out1, out2, out3, out4)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN)  :: F(3,3)
      DOUBLE COMPLEX, INTENT(IN)  :: thetat
      DOUBLE PRECISION, INTENT(IN) :: props(5)
      DOUBLE COMPLEX, INTENT(OUT) :: out1(3,3)
      DOUBLE COMPLEX, INTENT(OUT) :: out2(3,3)
      DOUBLE COMPLEX, INTENT(OUT) :: out3
      DOUBLE COMPLEX, INTENT(OUT) :: out4
      ...
      DOUBLE COMPLEX :: MM3(3,3)
      DOUBLE COMPLEX :: C(3,3)
      DOUBLE COMPLEX :: Cbar(3,3)
      DOUBLE COMPLEX :: INV4(3,3)
      DOUBLE COMPLEX :: Cbar_inv(3,3)
      DOUBLE COMPLEX :: S(3,3)
      DOUBLE COMPLEX :: theta
      DOUBLE COMPLEX :: J
      ...
      theta = (DCMPLX(1.000000000000000d+00, 0.0d0) + thetat)
      J = det33z(F)
      CALL transpose33z(F, AT2)
      CALL matmul33z(AT2, F, MM3)
      C = MM3
      Cbar = (((theta ** (DCMPLX(2.000000000000000d+00, 0.0d0) /
     &DCMPLX(3.000000000000000d+00, 0.0d0))) * (J ** ((-
     &DCMPLX(2.000000000000000d+00, 0.0d0)) /
     &DCMPLX(3.000000000000000d+00, 0.0d0)))) * C)
      CALL inv33z(Cbar, INV4)
      Cbar_inv = INV4
      a = ((DCMPLX(props(2), 0.0d0) * LOG(theta)) - DCMPLX(props(1),
     &0.0d0))
      S = ((DCMPLX(props(1), 0.0d0) * I) + (a * Cbar_inv))
      ...
      RETURN
      END SUBROUTINE scovazziblockmaterial_S_enriched

      SUBROUTINE scovazziblockmaterial_Dbar(Cbar_inv, theta, props,
     &result)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN)  :: Cbar_inv(3,3)
      DOUBLE COMPLEX, INTENT(IN)  :: theta
      DOUBLE PRECISION, INTENT(IN) :: props(5)
      DOUBLE COMPLEX, INTENT(OUT) :: result(3,3)
      ...
      factor = ((DCMPLX(3.000000000000000d+00, 0.0d0) * DCMPLX(props(2),
     & 0.0d0)) + (DCMPLX(2.000000000000000d+00, 0.0d0) *
     &DCMPLX(props(1), 0.0d0)))
      factor = (factor - ((DCMPLX(2.000000000000000d+00, 0.0d0) *
     &DCMPLX(props(2), 0.0d0)) * LOG(theta)))

      DO ii = 1, 3
        DO jj = 1, 3
          result(ii,jj) = (factor * Cbar_inv(ii,jj))
        END DO
      END DO

      RETURN
      END SUBROUTINE scovazziblockmaterial_Dbar


C======================================================================
C     Section 3: Complex-step tangent engine
C======================================================================
      SUBROUTINE scovazziblockmaterial_cs_tangents(
     &   F, thetat, grad_thetat, F_old, props, dPK1_dF,
     &   dPK1_dthetat, dphase_flux_dF, dphase_flux_dgrad_thetat,
     &   dphase_flux_dthetat, dphase_storage_dF,
     &   dphase_storage_dthetat)
...
C     --- Inputs (all DOUBLE PRECISION) ---
      ...
      DOUBLE PRECISION, INTENT(IN) :: thetat
      DOUBLE PRECISION, INTENT(IN) :: grad_thetat(3)
      DOUBLE PRECISION, INTENT(IN) :: F_old(3,3)
      DOUBLE PRECISION, INTENT(IN) :: props(5)

C     --- Outputs (all DOUBLE PRECISION) ---
      DOUBLE PRECISION, INTENT(OUT) :: dPK1_dF(3,3,3,3)
      DOUBLE PRECISION, INTENT(OUT) :: dPK1_dthetat(3,3)
      DOUBLE PRECISION, INTENT(OUT) :: dphase_flux_dF(3,3,3)
      DOUBLE PRECISION, INTENT(OUT) :: dphase_flux_dgrad_thetat(3,3)
      DOUBLE PRECISION, INTENT(OUT) :: dphase_flux_dthetat(3)
      DOUBLE PRECISION, INTENT(OUT) :: dphase_storage_dF(3,3)
      DOUBLE PRECISION, INTENT(OUT) :: dphase_storage_dthetat
...
C     small h gives machine-precision tangents.
      ...
      DOUBLE COMPLEX :: thetatz
      DOUBLE COMPLEX :: grad_thetat_z(3)
...
C     === dPK1_dF: d(stress_PK1)/d(F) ===
      DO k = 1, 3
        DO l = 1, 3
          ...
          thetatz = DCMPLX(thetat, 0.0d0)
          CALL real2complex3(grad_thetat, grad_thetat_z)
          Fz(k,l) = Fz(k,l) + DCMPLX(0.0d0, CS_H)
          CALL scovazziblockmaterial_stress_PK1(Fz, thetatz, props, PKz)
          DO i = 1, 3
            DO j = 1, 3
...
C     === dPK1_dthetat: d(stress_PK1)/d(thetat) ===
      CALL real2complex33(F, Fz)
      thetatz = DCMPLX(thetat, CS_H)
      CALL real2complex3(grad_thetat, grad_thetat_z)
      CALL scovazziblockmaterial_stress_PK1(Fz, thetatz, props, PKz)
      DO i = 1, 3
        DO j = 1, 3
          dPK1_dthetat(i,j) = AIMAG(PKz(i,j)) / CS_H
        END DO
      END DO
...
C     === dphase_flux_dF: d(phase_flux)/d(F) ===
      DO k = 1, 3
        DO l = 1, 3
          ...
          CALL real2complex3(grad_thetat, grad_thetat_z)
          Fz(k,l) = Fz(k,l) + DCMPLX(0.0d0, CS_H)
          CALL scovazziblockmaterial_phase_flux(Fz, thetatz,
     &grad_thetat_z, props, jRz)
          DO i = 1, 3
...
C     === dphase_flux_dgrad_thetat: d(phase_flux)/d(grad_thetat) ===
      DO k = 1, 3
        CALL real2complex33(F, Fz)
        thetatz = DCMPLX(thetat, 0.0d0)
        CALL real2complex3(grad_thetat, grad_thetat_z)
        grad_thetat_z(k) = grad_thetat_z(k) + DCMPLX(0.0d0, CS_H)
        CALL scovazziblockmaterial_phase_flux(Fz, thetatz,
     &grad_thetat_z, props, jRz)
        DO i = 1, 3
          dphase_flux_dgrad_thetat(i,k) = AIMAG(jRz(i)) / CS_H
        END DO
      END DO

C     === dphase_flux_dthetat: d(phase_flux)/d(thetat) ===
      CALL real2complex33(F, Fz)
      thetatz = DCMPLX(thetat, CS_H)
      CALL real2complex3(grad_thetat, grad_thetat_z)
      CALL scovazziblockmaterial_phase_flux(Fz, thetatz, grad_thetat_z,
     &props, jRz)
      DO i = 1, 3
        dphase_flux_dthetat(i) = AIMAG(jRz(i)) / CS_H
      END DO
...
C     === dphase_storage_dF: d(phase_storage)/d(F) ===
      DO k = 1, 3
        DO l = 1, 3
          ...
          CALL real2complex3(grad_thetat, grad_thetat_z)
          Fz(k,l) = Fz(k,l) + DCMPLX(0.0d0, CS_H)
          CALL scovazziblockmaterial_phase_storage(Fz, thetatz, props,
     &cdotz)
          dphase_storage_dF(k,l) = AIMAG(cdotz) / CS_H
        END DO
      END DO

C     === dphase_storage_dthetat: d(phase_storage)/d(thetat) ===
      CALL real2complex33(F, Fz)
      thetatz = DCMPLX(thetat, CS_H)
      CALL real2complex3(grad_thetat, grad_thetat_z)
      CALL scovazziblockmaterial_phase_storage(Fz, thetatz, props,
     &cdotz)
      dphase_storage_dthetat = AIMAG(cdotz) / CS_H

      RETURN
      END SUBROUTINE scovazziblockmaterial_cs_tangents
