...
C======================================================================
      ...
      IMPLICIT NONE
      INTEGER, PARAMETER :: LOCALP_UVARM_OFFSET = 1000000
      INTEGER, PARAMETER :: LOCALP_MAX_ELEM = 54000
      INTEGER, PARAMETER :: LOCALP_MAX_GP = 8
      INTEGER, PARAMETER :: LOCALP_NUVAR = 2
      DOUBLE PRECISION, SAVE ::
     &  LOCALP_UVAR(LOCALP_MAX_ELEM,LOCALP_MAX_GP,LOCALP_NUVAR)
      DATA LOCALP_UVAR /864000*0.0D0/
      CONTAINS
      SUBROUTINE localp_store_uvarm(noel, npt, phi, p)
      IMPLICIT NONE
      INTEGER, INTENT(IN) :: noel, npt
      DOUBLE PRECISION, INTENT(IN) :: phi, p
      IF (noel .LT. 1 .OR. noel .GT. LOCALP_MAX_ELEM) RETURN
      ...
      IF (src .LT. 1 .OR. src .GT. LOCALP_MAX_ELEM) RETURN
      IF (npt .LT. 1 .OR. npt .GT. LOCALP_MAX_GP) RETURN
...
C     Local Newton for element pressure
     ...
     &      dflux_dgrad_mu, dstorage_dF, dstorage_dp)
        ...
        dp = -R_p / K_pp
        p_elem = p_elem + dp
        IF (DABS(R_p) .LT. 1.0d-10) EXIT
        IF (DABS(dp) .LT. 1.0d-10*(1.0d0+DABS(p_elem))) EXIT
      END DO
...
C======================================================================
      SUBROUTINE chesteranandgel_stress_PK1(F, p, mu, props, P_out)
      IMPLICIT NONE
     ...
     &jj))) + (p * AT4(ii,jj)))
      ...
      RETURN
      END SUBROUTINE chesteranandgel_stress_PK1

      SUBROUTINE chesteranandgel_pressure_resid(F, p, mu, props, rp_out)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN) :: F(3,3)
      DOUBLE COMPLEX, INTENT(IN) :: p
      DOUBLE COMPLEX, INTENT(IN) :: mu
     ...
     &(((phi / DCMPLX(props(9), 0.0d0)) * DCMPLX(props(6), 0.0d0)) * p))
      ...
      RETURN
      END SUBROUTINE chesteranandgel_pressure_resid

      SUBROUTINE chesteranandgel_solvent_flux(F, p, mu, grad_mu, props,
     &jR_out)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN) :: F(3,3)
      DOUBLE COMPLEX, INTENT(IN) :: p
     ...
     &DCMPLX(props(8), 0.0d0)))
      ...
      RETURN
      END SUBROUTINE chesteranandgel_solvent_flux

      SUBROUTINE chesteranandgel_solvent_storage(F, F_old, p, p_old, dt,
     & props, cdot_out)
      IMPLICIT NONE
      DOUBLE COMPLEX, INTENT(IN) :: F(3,3)
     ...
     &0.0d0) * dt))
      ...
      RETURN
      END SUBROUTINE chesteranandgel_solvent_storage
...
C     Real outputs
      ...
      CALL chesteranandgel_stress_PK1(Fz, p_scalar_z, muz, PROPS,
     &stress_z)
      CALL chesteranandgel_pressure_resid(Fz, p_scalar_z, muz, PROPS,
     &rp_z)
      CALL chesteranandgel_solvent_flux(Fz, p_scalar_z, muz, grad_mu_z,
     &PROPS, flux_z)
      CALL chesteranandgel_solvent_storage(Fz, F_old_z, p_scalar_z,
...
C     F perturbations
      DO k = 1, 3
        DO l = 1, 3
          ...
          CALL chesteranandgel_stress_PK1(Fz, p_scalar_z, muz, PROPS,
     &stress_z)
          CALL chesteranandgel_pressure_resid(Fz, p_scalar_z, muz,
     &PROPS, rp_z)
          CALL chesteranandgel_solvent_flux(Fz, p_scalar_z, muz,
     &grad_mu_z, PROPS, flux_z)
          CALL chesteranandgel_solvent_storage(Fz, F_old_z, p_scalar_z,
     &p_old_z, dt, PROPS, cdot_z)
          DO i = 1, 3
